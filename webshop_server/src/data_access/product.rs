...
pub async fn get_product_by_id(pool: &Pool<Postgres>, product_id: &str) -> Result<Product, sqlx::Error> {
    let product = query_as!(Product, "SELECT * FROM product WHERE product_id = $1", product_id)
        .fetch_one(pool)
        .await?;
    Ok(product)
}